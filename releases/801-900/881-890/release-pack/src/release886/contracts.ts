export interface EnergyCarbonAndSustainabilityMeasurement {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_886_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateEnergyCarbonAndSustainabilityMeasurement(value: EnergyCarbonAndSustainabilityMeasurement): string[] {
  const errors: string[] = [];
