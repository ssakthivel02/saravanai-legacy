export interface ProviderContractExitAndPortabilityReadiness {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_888_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "provider_allowlist_required"] as const;

export function validateProviderContractExitAndPortabilityReadiness(value: ProviderContractExitAndPortabilityReadiness): string[] {
  const errors: string[] = [];
