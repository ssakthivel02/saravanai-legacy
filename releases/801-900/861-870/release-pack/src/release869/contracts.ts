export interface RegionalIncidentNotificationAndSupport {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_869_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "jurisdiction_control_required"] as const;

export function validateRegionalIncidentNotificationAndSupport(value: RegionalIncidentNotificationAndSupport): string[] {
  const errors: string[] = [];
