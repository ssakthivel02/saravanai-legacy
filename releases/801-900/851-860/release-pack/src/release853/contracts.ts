export interface ContinuousControlEvidenceCollectorRuntime {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_853_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden"] as const;

export function validateContinuousControlEvidenceCollectorRuntime(value: ContinuousControlEvidenceCollectorRuntime): string[] {
  const errors: string[] = [];
