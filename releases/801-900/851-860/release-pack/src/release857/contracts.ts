export interface ComplianceObligationChangeMonitor {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_857_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateComplianceObligationChangeMonitor(value: ComplianceObligationChangeMonitor): string[] {
  const errors: string[] = [];
