export interface AgentSandboxNetworkAndFileBoundary {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_826_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "bounded_agent_authority_required", "sandbox_isolation_required"] as const;

export function validateAgentSandboxNetworkAndFileBoundary(value: AgentSandboxNetworkAndFileBoundary): string[] {
  const errors: string[] = [];
