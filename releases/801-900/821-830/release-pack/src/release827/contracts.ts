export interface CompensatingActionAndRollbackExecutor {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_827_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden"] as const;

export function validateCompensatingActionAndRollbackExecutor(value: CompensatingActionAndRollbackExecutor): string[] {
  const errors: string[] = [];
