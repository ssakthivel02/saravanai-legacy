export interface ToolLeaseAndScopedCapabilityRuntime {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_823_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "tool_lease_required"] as const;

export function validateToolLeaseAndScopedCapabilityRuntime(value: ToolLeaseAndScopedCapabilityRuntime): string[] {
  const errors: string[] = [];
