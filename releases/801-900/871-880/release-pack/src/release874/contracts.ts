export interface SimulationExecutionSandboxRuntime {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_874_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "sandbox_isolation_required", "uncertainty_disclosure_required"] as const;

export function validateSimulationExecutionSandboxRuntime(value: SimulationExecutionSandboxRuntime): string[] {
  const errors: string[] = [];
