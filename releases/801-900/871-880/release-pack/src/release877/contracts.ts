export interface ResilienceFailureAndRecoverySimulation {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_877_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "uncertainty_disclosure_required"] as const;

export function validateResilienceFailureAndRecoverySimulation(value: ResilienceFailureAndRecoverySimulation): string[] {
  const errors: string[] = [];
