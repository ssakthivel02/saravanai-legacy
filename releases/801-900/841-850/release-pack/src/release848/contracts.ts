export interface WorkspaceSearchAndKnowledgeAssistance {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_848_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "source_authority_required", "workspace_portability_required"] as const;

export function validateWorkspaceSearchAndKnowledgeAssistance(value: WorkspaceSearchAndKnowledgeAssistance): string[] {
  const errors: string[] = [];
