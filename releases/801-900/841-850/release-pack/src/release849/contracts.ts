export interface WorkspaceExportDeletionAndPortability {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_849_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "workspace_portability_required"] as const;

export function validateWorkspaceExportDeletionAndPortability(value: WorkspaceExportDeletionAndPortability): string[] {
  const errors: string[] = [];
