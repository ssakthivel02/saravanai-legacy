export interface ProductionChangeApprovalAndLaunchWindow {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_898_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "separate_production_approval_required"] as const;

export function validateProductionChangeApprovalAndLaunchWindow(value: ProductionChangeApprovalAndLaunchWindow): string[] {
  const errors: string[] = [];
