export interface TenantScopedIndexAndRetrievalRuntime {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_833_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "tenant_isolation_required"] as const;

export function validateTenantScopedIndexAndRetrievalRuntime(value: TenantScopedIndexAndRetrievalRuntime): string[] {
  const errors: string[] = [];
