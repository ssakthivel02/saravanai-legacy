export interface KnowledgeCorrectionReindexAndNotification {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_839_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "source_authority_required"] as const;

export function validateKnowledgeCorrectionReindexAndNotification(value: KnowledgeCorrectionReindexAndNotification): string[] {
  const errors: string[] = [];
