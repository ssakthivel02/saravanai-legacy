export interface DocumentIngestionAndQuarantineWorker {
  requestId: string;
  tenantId: string;
  owner: string;
  inputRef: string;
...
export const RELEASE_832_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "strict_input_schema_required", "idempotency_required", "result_evidence_required"] as const;

export function validateDocumentIngestionAndQuarantineWorker(value: DocumentIngestionAndQuarantineWorker): string[] {
  const errors: string[] = [];
