export interface ResearchSynthesisAndReportPipeline {
  requestId: string;
  tenantId: string;
  owner: string;
  inputRef: string;
...
export const RELEASE_838_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "strict_input_schema_required", "idempotency_required", "result_evidence_required"] as const;

export function validateResearchSynthesisAndReportPipeline(value: ResearchSynthesisAndReportPipeline): string[] {
  const errors: string[] = [];
