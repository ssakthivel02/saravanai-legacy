export interface TemporalFactVerificationRuntime {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_836_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "fresh_source_verification_required"] as const;

export function validateTemporalFactVerificationRuntime(value: TemporalFactVerificationRuntime): string[] {
  const errors: string[] = [];
