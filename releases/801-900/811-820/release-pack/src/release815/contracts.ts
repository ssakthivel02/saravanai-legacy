export interface OutputValidationAndSafetyPipeline {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_815_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "output_validation_required"] as const;

export function validateOutputValidationAndSafetyPipeline(value: OutputValidationAndSafetyPipeline): string[] {
  const errors: string[] = [];
