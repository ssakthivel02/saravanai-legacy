export interface ProviderHealthCircuitBreakerAndFallback {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_816_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "provider_allowlist_required"] as const;

export function validateProviderHealthCircuitBreakerAndFallback(value: ProviderHealthCircuitBreakerAndFallback): string[] {
  const errors: string[] = [];
