export interface AICostCapacityAndQueueController {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_818_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "free_first_hard_stop_required"] as const;

export function validateAICostCapacityAndQueueController(value: AICostCapacityAndQueueController): string[] {
  const errors: string[] = [];
