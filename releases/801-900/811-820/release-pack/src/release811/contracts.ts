export interface AIGatewayRequestEnvelope {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_811_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "ai_gateway_policy_required"] as const;

export function validateAIGatewayRequestEnvelope(value: AIGatewayRequestEnvelope): string[] {
  const errors: string[] = [];
