export interface FreeFirstModelRoutingRuntime {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_813_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden"] as const;

export function validateFreeFirstModelRoutingRuntime(value: FreeFirstModelRoutingRuntime): string[] {
  const errors: string[] = [];
