export interface RuntimeIdentityContextResolver {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_801_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "trusted_identity_context_required"] as const;

export function validateRuntimeIdentityContextResolver(value: RuntimeIdentityContextResolver): string[] {
  const errors: string[] = [];
