export interface IdentityRuntimeObservabilityAndAbuseDetection {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_809_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required", "trusted_identity_context_required"] as const;

export function validateIdentityRuntimeObservabilityAndAbuseDetection(value: IdentityRuntimeObservabilityAndAbuseDetection): string[] {
  const errors: string[] = [];
