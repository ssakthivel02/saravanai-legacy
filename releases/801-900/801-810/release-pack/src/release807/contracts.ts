export interface SessionRevocationAndDeviceTrust {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_807_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "unsupported_certification_claims_forbidden"] as const;

export function validateSessionRevocationAndDeviceTrust(value: SessionRevocationAndDeviceTrust): string[] {
  const errors: string[] = [];
