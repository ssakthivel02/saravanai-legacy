export interface PrivilegedSessionAndStepUpControl {
  executionId: string;
  tenantId: string;
  actorSubject: string;
  purpose: string;
...
export const RELEASE_805_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "phishing_resistant_step_up_required"] as const;

export function validatePrivilegedSessionAndStepUpControl(value: PrivilegedSessionAndStepUpControl): string[] {
  const errors: string[] = [];
