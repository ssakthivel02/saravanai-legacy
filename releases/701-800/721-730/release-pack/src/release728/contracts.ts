export interface ApplicationQualitySecurityAndAccessibilityTesting {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_728_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required", "secure_sdlc_required", "accessibility_review_required"] as const;

export function validateApplicationQualitySecurityAndAccessibilityTesting(value: ApplicationQualitySecurityAndAccessibilityTesting): string[] {
  const errors: string[] = [];
