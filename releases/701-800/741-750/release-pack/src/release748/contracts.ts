export interface SkillsTaxonomyAndRoleMapping {
  recordId: string;
  tenantId: string;
  name: string;
  owner: string;
...
export const RELEASE_748_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "provenance_required", "lifecycle_governance_required"] as const;

export function validateSkillsTaxonomyAndRoleMapping(value: SkillsTaxonomyAndRoleMapping): string[] {
  const errors: string[] = [];
