export interface InteractiveLessonAndPracticeGenerator {
  executionId: string;
  tenantId: string;
  owner: string;
...
export const RELEASE_743_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden"] as const;

export function validateInteractiveLessonAndPracticeGenerator(value: InteractiveLessonAndPracticeGenerator): string[] {
  const errors: string[] = [];
