export interface DataProductsAndSemanticLayerAssuranceGate {
  gateId: string;
  releaseRange: '771-780';
...
export const RELEASE_780_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "evidence_index_hash_required", "residual_risks_recorded", "no_go_supported", "data_classification_required", "semantic_versioning_required"] as const;

export function validateDataProductsAndSemanticLayerAssuranceGate(value: DataProductsAndSemanticLayerAssuranceGate): string[] {
  const errors: string[] = [];
