export interface InfrastructureChangeAndMigrationOrchestration {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_769_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "infrastructure_change_control_required", "migration_rehearsal_required"] as const;

export function validateInfrastructureChangeAndMigrationOrchestration(value: InfrastructureChangeAndMigrationOrchestration): string[] {
  const errors: string[] = [];
