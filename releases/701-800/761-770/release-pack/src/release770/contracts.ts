export interface HybridCloudAndInfrastructureAssuranceGate {
  gateId: string;
  releaseRange: '761-770';
...
export const RELEASE_770_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "evidence_index_hash_required", "residual_risks_recorded", "no_go_supported", "cloud_policy_required", "infrastructure_change_control_required"] as const;

export function validateHybridCloudAndInfrastructureAssuranceGate(value: HybridCloudAndInfrastructureAssuranceGate): string[] {
  const errors: string[] = [];
