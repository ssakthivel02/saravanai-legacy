export interface ModelRegressionAndReleaseComparison {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_708_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateModelRegressionAndReleaseComparison(value: ModelRegressionAndReleaseComparison): string[] {
  const errors: string[] = [];
