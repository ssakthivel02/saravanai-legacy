export interface HallucinationAndFactualityEvaluationV2 {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_705_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required", "protected_baseline_required", "source_grounding_required"] as const;

export function validateHallucinationAndFactualityEvaluationV2(value: HallucinationAndFactualityEvaluationV2): string[] {
  const errors: string[] = [];
