export interface MultimodalProjectAndAssetWorkspace {
  recordId: string;
  tenantId: string;
  name: string;
  owner: string;
...
export const RELEASE_641_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "provenance_required", "lifecycle_governance_required"] as const;

export function validateMultimodalProjectAndAssetWorkspace(value: MultimodalProjectAndAssetWorkspace): string[] {
  const errors: string[] = [];
