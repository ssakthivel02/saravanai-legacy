export interface MediaQualityAndBrandCompliance {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_646_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required", "synthetic_media_disclosure_required", "qualified_interpretation_required"] as const;

export function validateMediaQualityAndBrandCompliance(value: MediaQualityAndBrandCompliance): string[] {
  const errors: string[] = [];
