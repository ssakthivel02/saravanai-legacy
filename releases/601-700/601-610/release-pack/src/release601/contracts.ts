export interface AIRuntimeServiceRegistry {
  recordId: string;
  tenantId: string;
  name: string;
  owner: string;
...
export const RELEASE_601_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "provenance_required", "lifecycle_governance_required", "runtime_isolation_required"] as const;

export function validateAIRuntimeServiceRegistry(value: AIRuntimeServiceRegistry): string[] {
  const errors: string[] = [];
