export interface KnowledgeFeedbackAndCorrectionWorkflow {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_629_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "knowledge_freshness_required", "workflow_exception_path_required"] as const;

export function validateKnowledgeFeedbackAndCorrectionWorkflow(value: KnowledgeFeedbackAndCorrectionWorkflow): string[] {
  const errors: string[] = [];
