export interface SecureReleaseArtefactRegistry {
  recordId: string;
  tenantId: string;
  name: string;
  owner: string;
...
export const RELEASE_658_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "provenance_required", "lifecycle_governance_required"] as const;

export function validateSecureReleaseArtefactRegistry(value: SecureReleaseArtefactRegistry): string[] {
  const errors: string[] = [];
