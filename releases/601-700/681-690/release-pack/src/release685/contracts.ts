export interface IssueFindingAndRemediationManagement {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_685_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "synthetic_media_disclosure_required", "automated_remediation_bounded"] as const;

export function validateIssueFindingAndRemediationManagement(value: IssueFindingAndRemediationManagement): string[] {
  const errors: string[] = [];
