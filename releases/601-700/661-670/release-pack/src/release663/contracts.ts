export interface SLOErrorBudgetAndReliabilityPolicy {
  policyId: string;
  version: string;
  owner: string;
  ruleIds: string[];
...
export const RELEASE_663_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "tests_before_approval", "exception_governance_required"] as const;

export function validateSLOErrorBudgetAndReliabilityPolicy(value: SLOErrorBudgetAndReliabilityPolicy): string[] {
  const errors: string[] = [];
