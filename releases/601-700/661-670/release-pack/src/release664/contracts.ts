export interface AnomalyDetectionAndAlertQuality {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_664_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateAnomalyDetectionAndAlertQuality(value: AnomalyDetectionAndAlertQuality): string[] {
  const errors: string[] = [];
