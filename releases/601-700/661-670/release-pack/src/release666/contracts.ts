export interface AutomatedRemediationSafetyController {
  executionId: string;
  tenantId: string;
  owner: string;
...
export const RELEASE_666_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "idempotency_required", "kill_switch_required", "production_write_forbidden", "synthetic_media_disclosure_required", "automated_remediation_bounded"] as const;

export function validateAutomatedRemediationSafetyController(value: AutomatedRemediationSafetyController): string[] {
  const errors: string[] = [];
