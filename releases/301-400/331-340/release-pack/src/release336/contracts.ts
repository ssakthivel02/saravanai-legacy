export interface GoldenPathComplianceScoring {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_336_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateGoldenPathComplianceScoring(value: GoldenPathComplianceScoring): string[] {
  const errors: string[] = [];
