export interface InfrastructureModuleRegistry {
  recordId: string;
  tenantId: string;
  name: string;
  owner: string;
...
export const RELEASE_333_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "provenance_required", "lifecycle_governance_required"] as const;

export function validateInfrastructureModuleRegistry(value: InfrastructureModuleRegistry): string[] {
  const errors: string[] = [];
