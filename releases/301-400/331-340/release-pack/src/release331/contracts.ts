export interface PlatformAPIProductCatalogue {
  recordId: string;
  tenantId: string;
  name: string;
  owner: string;
...
export const RELEASE_331_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "provenance_required", "lifecycle_governance_required"] as const;

export function validatePlatformAPIProductCatalogue(value: PlatformAPIProductCatalogue): string[] {
  const errors: string[] = [];
