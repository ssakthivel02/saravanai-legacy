export interface ResponsibleDataValueAndSharingReadiness {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_349_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateResponsibleDataValueAndSharingReadiness(value: ResponsibleDataValueAndSharingReadiness): string[] {
  const errors: string[] = [];
