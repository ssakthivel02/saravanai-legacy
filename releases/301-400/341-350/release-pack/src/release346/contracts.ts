export interface SecureMultiPartyComputationReadiness {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_346_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateSecureMultiPartyComputationReadiness(value: SecureMultiPartyComputationReadiness): string[] {
  const errors: string[] = [];
