export interface SustainabilityAndCapacityTradeOffGovernance {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_379_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required", "methodology_disclosure_required"] as const;

export function validateSustainabilityAndCapacityTradeOffGovernance(value: SustainabilityAndCapacityTradeOffGovernance): string[] {
  const errors: string[] = [];
