export interface RegionalActiveActiveReadiness {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_372_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateRegionalActiveActiveReadiness(value: RegionalActiveActiveReadiness): string[] {
  const errors: string[] = [];
