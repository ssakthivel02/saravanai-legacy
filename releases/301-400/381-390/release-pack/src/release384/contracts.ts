export interface AccessibleContentAndAssistiveTechnologyTesting {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_384_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required", "accessibility_review_required"] as const;

export function validateAccessibleContentAndAssistiveTechnologyTesting(value: AccessibleContentAndAssistiveTechnologyTesting): string[] {
  const errors: string[] = [];
