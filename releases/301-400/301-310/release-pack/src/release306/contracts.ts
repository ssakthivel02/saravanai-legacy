export interface ThreatAndAttackSimulation {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_306_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required", "isolated_simulation_required"] as const;

export function validateThreatAndAttackSimulation(value: ThreatAndAttackSimulation): string[] {
  const errors: string[] = [];
