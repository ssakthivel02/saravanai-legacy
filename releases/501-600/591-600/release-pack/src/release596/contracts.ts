export interface SupportTrainingAndCertificationReadiness {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_596_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "certification_not_claimed"] as const;

export function validateSupportTrainingAndCertificationReadiness(value: SupportTrainingAndCertificationReadiness): string[] {
  const errors: string[] = [];
