export interface ValueTransparencyAndExecutiveReporting {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_579_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateValueTransparencyAndExecutiveReporting(value: ValueTransparencyAndExecutiveReporting): string[] {
  const errors: string[] = [];
