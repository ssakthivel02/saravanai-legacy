export interface BenefitsRealisationAndOutcomeVerification {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_575_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateBenefitsRealisationAndOutcomeVerification(value: BenefitsRealisationAndOutcomeVerification): string[] {
  const errors: string[] = [];
