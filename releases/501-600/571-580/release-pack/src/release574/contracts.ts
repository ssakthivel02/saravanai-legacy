export interface InvestmentPortfolioPrioritisation {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_574_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateInvestmentPortfolioPrioritisation(value: InvestmentPortfolioPrioritisation): string[] {
  const errors: string[] = [];
