export interface DecisionAppealAndReconsideration {
  planId: string;
  tenantId: string;
  owner: string;
  objective: string;
...
export const RELEASE_509_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "dependencies_required", "milestones_required", "rollback_or_exit_required", "decision_traceability_required", "appeal_and_redress_required"] as const;

export function validateDecisionAppealAndReconsideration(value: DecisionAppealAndReconsideration): string[] {
  const errors: string[] = [];
