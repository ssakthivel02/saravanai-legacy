export interface EventContractAndSchemaRegistry {
  recordId: string;
  tenantId: string;
  name: string;
  owner: string;
...
export const RELEASE_522_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "provenance_required", "lifecycle_governance_required"] as const;

export function validateEventContractAndSchemaRegistry(value: EventContractAndSchemaRegistry): string[] {
  const errors: string[] = [];
