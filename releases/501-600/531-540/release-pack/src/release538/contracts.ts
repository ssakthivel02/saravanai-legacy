export interface CyberCrisisExerciseAndLessons {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_538_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateCyberCrisisExerciseAndLessons(value: CyberCrisisExerciseAndLessons): string[] {
  const errors: string[] = [];
