export interface EnterpriseDigitalAssetRegistry {
  recordId: string;
  tenantId: string;
  name: string;
  owner: string;
...
export const RELEASE_511_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "provenance_required", "lifecycle_governance_required"] as const;

export function validateEnterpriseDigitalAssetRegistry(value: EnterpriseDigitalAssetRegistry): string[] {
  const errors: string[] = [];
