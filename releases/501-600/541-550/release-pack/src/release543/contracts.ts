export interface PasswordlessAndPasskeyReadiness {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_543_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validatePasswordlessAndPasskeyReadiness(value: PasswordlessAndPasskeyReadiness): string[] {
  const errors: string[] = [];
