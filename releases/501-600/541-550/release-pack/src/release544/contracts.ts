export interface VerifiableCredentialReadiness {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_544_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateVerifiableCredentialReadiness(value: VerifiableCredentialReadiness): string[] {
  const errors: string[] = [];
