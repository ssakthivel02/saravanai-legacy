export interface CustomerIdentityAndPreferenceCentre {
  profileId: string;
  tenantId: string;
  owner: string;
  policyRefs: string[];
...
export const RELEASE_452_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "tenant_scope_required", "policy_binding_required", "identity_assurance_required", "customer_redress_required"] as const;

export function validateCustomerIdentityAndPreferenceCentre(value: CustomerIdentityAndPreferenceCentre): string[] {
  const errors: string[] = [];
