export interface ExposureAndVulnerabilityPrioritisation {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_426_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateExposureAndVulnerabilityPrioritisation(value: ExposureAndVulnerabilityPrioritisation): string[] {
  const errors: string[] = [];
