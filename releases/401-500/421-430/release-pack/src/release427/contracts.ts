export interface IdentityThreatDetectionAndResponseV2 {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_427_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required", "threat_intelligence_handling_required", "identity_assurance_required"] as const;

export function validateIdentityThreatDetectionAndResponseV2(value: IdentityThreatDetectionAndResponseV2): string[] {
  const errors: string[] = [];
