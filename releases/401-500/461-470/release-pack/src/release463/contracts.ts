export interface EnergyEfficiencyEngineering {
  assessmentId: string;
  tenantId: string;
  subjectId: string;
  owner: string;
...
export const RELEASE_463_CONTROLS = ["owner_accountability_required", "evidence_integrity_required", "methodology_required", "findings_review_required"] as const;

export function validateEnergyEfficiencyEngineering(value: EnergyEfficiencyEngineering): string[] {
  const errors: string[] = [];
