...
const required = [
  ...
  'src/access-jwt.js',
  'src/owner-api.js',
  'assets/access-role-policy.js',
  'assets/access-role-policy.css',
  'assets/release-labels.js',
  'service-worker.js',
  'openapi/sakthiai-v1.yaml',
  'tests/access-policy.test.mjs',
  'tests/access-role-policy-ui.test.mjs',
  'tests/access-jwt.test.mjs',
  'tests/platform-release-contract.test.mjs',
  'docs/BUILD_016_ACCESS_ROLE_POLICY.md',
  'ACCESS_ROLE_POLICY_BASELINE.json'
];

for (const path of required) {
  if (!fs.existsSync(path)) throw new Error(`Missing Build 016 file: ${path}`);
}
...
for (const marker of [
  'access-role-policy-foundation-1.0.0',
  'ACCESS_TEAM_PROFILES_ENABLED',
  'ACCESS_READER_PROFILES_ENABLED',
  'ACCESS_ROLE_POLICY_INVALID',
  'ACCESS_TEAM_PROFILES_DISABLED',
  'ACCESS_READER_PROFILES_DISABLED',
...
if (!labels.includes('Owner build 016')) throw new Error('Owner Build 016 label is missing.');
for (const asset of ['/assets/access-role-policy.js', '/assets/access-role-policy.css']) {
  if (!serviceWorker.includes(asset)) throw new Error(`Service worker does not cache ${asset}`);
}
if (!serviceWorker.includes('sakthiai-owner-v16-role-policy')) throw new Error('Build 016 PWA cache rotation is missing.');
if (!openapi.includes('/api/v1/platform/access/readiness:') || !openapi.includes('0.16.0-role-policy')) {
  throw new Error('OpenAPI Build 016 role-policy contract is missing.');
}
...
for (const variable of [
  ...
  const pattern = new RegExp(`"${variable}"\\s*:\\s*"true"`);
  if (pattern.test(wrangler)) throw new Error(`Repository default must not enable ${variable}.`);
}
...
if (baseline.cost.paidServicesRequired !== false || baseline.cost.paidFallbackEnabled !== false) {
  throw new Error('Build 016 must not require or silently activate paid services.');
}

for (const marker of [
  'Public registration remains disabled',
  'Team profiles remain disabled by default',
  'Reader profiles remain disabled by default',
  'No authentication setting is activated automatically'
]) {
  if (!docs.includes(marker)) throw new Error(`Build 016 documentation missing safety statement: ${marker}`);
}

console.log('Build 016 access role policy, owner-first admission and safety validation passed.');
